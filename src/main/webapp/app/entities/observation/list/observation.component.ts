...
import { EntityArrayResponseType, ObservationService } from '../service/observation.service';
...
export class ObservationComponent implements OnInit {
  ...
  trackId = (_index: number, item: IObservation): number => this.observationService.getObservationIdentifier(item);

  search(query: string): void {
    if (query && ObservationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (ObservationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.observations = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IObservation[] | null): IObservation[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.observationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.observationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
