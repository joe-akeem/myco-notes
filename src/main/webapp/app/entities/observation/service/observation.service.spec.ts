...
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../observation.test-samples';

import { ObservationService, RestObservation } from './observation.service';

const requireRestSample: RestObservation = {
  ...sampleWithRequiredData,
  observationDate: sampleWithRequiredData.observationDate?.format(DATE_FORMAT),
};
...
describe('Observation Service', () => {
  ...
  describe('Service methods', () => {
    ...
    it('should create a Observation', () => {
      ...
      const observation = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(observation).subscribe(resp => (expectedResult = resp.body));
    ...
    it('should update a Observation', () => {
      const observation = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(observation).subscribe(resp => (expectedResult = resp.body));
    ...
    describe('addObservationToCollectionIfMissing', () => {
      it('should add a Observation to an empty array', () => {
        const observation: IObservation = sampleWithRequiredData;
      ...
      it("should add a Observation to an array that doesn't contain it", () => {
        const observation: IObservation = sampleWithRequiredData;
        const observationCollection: IObservation[] = [sampleWithPartialData];
        expectedResult = service.addObservationToCollectionIfMissing(observationCollection, observation);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(observation);
      });
      ...
      it('should add only unique Observation to an array', () => {
        const observationArray: IObservation[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const observationCollection: IObservation[] = [sampleWithRequiredData];
      ...
      it('should accept varargs', () => {
        const observation: IObservation = sampleWithRequiredData;
        const observation2: IObservation = sampleWithPartialData;
    ...
    describe('compareObservation', () => {
      it('Should return true if both entities are null', () => {
        ...
        const compareResult = service.compareObservation(entity1, entity2);
      ...
      it('Should return false if one entity is null', () => {
        ...
        const compareResult1 = service.compareObservation(entity1, entity2);
        const compareResult2 = service.compareObservation(entity2, entity1);
